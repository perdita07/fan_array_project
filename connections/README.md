...
Each fan in the array is wired the same way — power from the SMPS, PWM control from the Arduino Mega, and a tachometer line pulled up to 5V through a 10 kΩ resistor into an interrupt pin for RPM feedback. This repeats ×9 across the array (see Chapter 3, System Architecture, in `project_report.docx` for the full firmware/hardware writeup):

![Per-fan wiring schematic](./docs/wiring_diagram.svg)
...
Raw data, reference papers, and readings are provided separately here: **[Additional resources (Google Drive)](https://drive.google.com/drive/folders/1X09Ndj4WAZ8-77vm1_YXIhqzWv36yzKz?usp=sharing)**

The notebooks expect raw sensor exports (multimeter/tachometer logs, anemometer voltage logs) as `.txt` or `.xlsx` files with names like `z=2,r=0.xlsx` and `fan_log.csv`. Download the relevant files from the Drive folder above, then either:
- upload them via the `files.upload()` step in `01_Data_Preprocessing.ipynb` when running in Colab, or
- place them in a local `data/` folder and use `config.py` (see below) so paths don't need to be hardcoded per notebook.

### Centralised paths (`config.py`)

A small `config.py` is included to avoid hardcoding Colab-style `/content/...` paths or bare filenames inside every notebook. Point it at your data with an environment variable:

```bash
export FAN_ARRAY_DATA_DIR=/path/to/downloaded/drive/folder
```

or just drop your files into a `data/` folder next to `config.py` (the default). Then, at the top of each notebook:
...
`data_path()` raises a clear error naming the missing file if it isn't where expected, instead of failing deep inside pandas. This is a starting point, not a full refactor — see the "Maintainability Recommendations" section of `REPO_STRUCTURE_PLAN.md` for the other suggested extractions (shared data-loading helper, shared fit-and-score helper, etc.) that haven't been applied yet.
...
├── docs/
│   └── wiring_diagram.svg     # per-fan wiring schematic
├── config.py                  # centralised data paths (see Data section above)
├── requirements.txt
├── data/                       # not tracked in git — populate from the Drive link above
├── project_report.docx
...
This work is licensed under **[CC BY-NC-ND 4.0](https://creativecommons.org/licenses/by-nc-nd/4.0/)** (Attribution–NonCommercial–NoDerivatives). You're welcome to view and share this work with credit, but non-commercial use only, and no modified/derivative versions may be redistributed. This project reflects unpublished academic work — for reuse beyond what the license permits, please contact the author.
